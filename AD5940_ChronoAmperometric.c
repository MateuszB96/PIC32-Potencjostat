...
int32_t AMPShowResult(float *pData, uint32_t DataCount)
{
  ...
  {
    *(uint32_t*)(Bufor) = index++;  // Konwersja z float do bajtów
    *(float*)(Bufor+4) = pData[i];
    Send_USB_HID(Bufor);
  }
  return 0;
}
...
void AD5940AMPStructInit(int *NumData, float *SensorBias, float *Vzero, float *Amplitude, uint32_t *Length)
{
  ...
  /* Configure general parameters */
	...
	pAMPCfg->SensorBias = *SensorBias;                            /* Sensor bias voltage between reference and sense electrodes*/
	pAMPCfg->Vzero = *Vzero;
	/* Configure Pulse*/
	pAMPCfg->pulseAmplitude = *Amplitude;						/* Pulse amplitude on counter electrode (mV) */
	pAMPCfg->pulseLength = *Length;							    /* Length of voltage pulse in ms */
}

void AD5940_ChronoAmperometric_Main(int *NumData, float *SensorBias, \
                                    float *Vzero, float *Amplitude, uint32_t *Length)
{
  uint32_t temp[n];
  AppCHRONOAMPCfg_Type *pAMPCfg;
  AppCHRONOAMPGetCfg(&pAMPCfg);
  ...
  AD5940AMPStructInit(NumData,SensorBias,Vzero,Amplitude,Length); /* Configure your parameters in this function */
