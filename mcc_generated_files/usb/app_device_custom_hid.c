...
//#include "../PIC/PIC_Projekty/USB_HID.X/mcc_generated_files/system.h"
 ...
 */
    ...
    #endif
#else
    volatile unsigned char ReceivedDataBuffer[64];
    //unsigned char ToSendDataBuffer[64];
#endif
...
*
********************************************************************/
void Send_USB_HID(uint8_t ToSendDataBuffer[64])
{   
...
uint8_t Recive_USB_HID(uint8_t ReceivedDataBuffer[0]) {
    /* If the USB device isn't configured yet, we can't really do anything
     * else since we don't have a host to talk to.  So jump back to the
     * top of the while loop. */
    if (USBGetDeviceState() < CONFIGURED_STATE) {
