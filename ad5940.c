...
volatile static uint32_t Flaga = 0;       /* Flaga do sygnalizowania przerwania */
...
/* Slave Select (SS) Pin, zwi?zany z modu?em SPI */
...
/* AD5941 posiada pin resetu, który podlczony jest do MCU. 
   Ustawienie stanu niskiego pinu wywoluje reset sprzetowy AD5941.
 */
...
/* Funkcja sluzy do opoznienia o time / us */
...
{
    SPI1_Exchange8bitBuffer(pSendBuffer,length,pRecvBuff);
}
...
*/
void __attribute__ ( ( vector ( _EXTERNAL_3_VECTOR ), interrupt ( IPL1SOFT ) ) ) _EXTERNAL_2_ISR (void)
{
    //***User Area Begin->code: External 3***
	
	// EX_INT3 callback function 
	EX_INT2_CallBack();
