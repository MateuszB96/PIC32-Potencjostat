...
int32_t AMPerometricShowResult(float *pData, uint32_t DataCount)
{
...
void AD5940AMPerometricStructInit(float *SampleDelay, float *Vzero, float *SeBias, int *NumData)
{
  AppAMPCfg_Type *pAMPCfg;
    ...
    pAMPCfg->NumOfData = *NumData;      /* Never stop until you stop it manually by AppAMPCtrl() function */	
	
	
	/* Configure measurement parameters */
    pAMPCfg->AmpODR = *SampleDelay;          	    /* Time between samples in seconds */
    pAMPCfg->FifoThresh = 4;      		/* Number of measurements before alerting host microcontroller */
	
    pAMPCfg->SensorBias = *SeBias;   			/* Sensor bias voltage between reference and sense electrodes*/
	pAMPCfg->LptiaRtiaSel = LPTIARTIA_1K;
	pAMPCfg->LpTiaRl = LPTIARLOAD_10R;
	pAMPCfg->Vzero = *Vzero;        		/* Vzero voltage. Voltage on Sense electrode. Unit is mV*/
	
	pAMPCfg->ADCRefVolt = 1.82;		    /* Measure voltage on Vref_1V8 pin */
}

void AD5940_Amperometric_Main(float *SampleDelay, float *Vzero, float *SeBias, int *NumData)
{
  uint32_t temp;
  
  AD5940PlatformCfg();
  AD5940AMPerometricStructInit(SampleDelay,Vzero, SeBias, NumData); /* Configure your parameters in this function */ 
  ...
  while(1)
  {
    ...
    if(AD5940_GetMCUIntFlag())
    {
      ...
      AMPerometricShowResult((float*)AppBuff, temp); /* Show the results to UART */
    }
  }
}
