...
static int32_t RampShowResult(float *pData, uint32_t DataCount)
{
  ...
  {
    *(uint32_t*)(Bufor) = index++; // Konwersja z uint32_t do 4 bajtów
    *(float*)(Bufor+4) = pData[i]; // Konwersja z float do 4 bajtów
     Send_USB_HID(Bufor);
  }
 ...
 * @return return 0.
*/
void AD5940RampSqrtStructInit(float *RampStart, float *RampPeak, float *VzeroStart,\
        float *VzeroPeak, uint32_t *Freq, uint32_t *Amp, uint32_t *Inc, float *SampleDelay)
{
  ...
  pRampCfg->FifoThresh = 2;                   /* Maximum value is 2kB/4-1 = 512-1. Set it to higher value to save power. */
  ...
  pRampCfg->RampStartVolt = *RampStart;           /* -1.4V */
  pRampCfg->RampPeakVolt = *RampPeak;             /* -0.1V */
  pRampCfg->VzeroStart = *VzeroStart;               /* 1.3V */
  pRampCfg->VzeroPeak = *VzeroPeak;                /* 1.3V */
  pRampCfg->Frequency = *Freq;                     /* Frequency of square wave in Hz */
  pRampCfg->SqrWvAmplitude = *Amp;                /* Amplitude of square wave in mV */
  pRampCfg->SqrWvRampIncrement = *Inc;           /* Increment in mV*/
  pRampCfg->SampleDelay = *SampleDelay;          /* 10ms. Time between update DAC and ADC sample. Unit is ms. */
  pRampCfg->LPTIARtiaSel = LPTIARTIA_200R;      /* Maximum current decides RTIA value */
}

void AD5940_SqrtVoltametry_Main(float *RampStart, float *RampPeak, float *VzeroStart,\
        float *VzeroPeak, uint32_t *Freq, uint32_t *Amp, uint32_t *Inc, float *SampleDelay)
{
  uint32_t temp;  
  AD5940PlatformCfg();
  AD5940RampSqrtStructInit(RampStart, RampPeak, VzeroStart, VzeroPeak, \
                           Freq, Amp, Inc, SampleDelay);
